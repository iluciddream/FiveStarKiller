...
def capture_packets(nic=None, protocol=None, src_addr=None, dst_addr=None):
    ...
    packets = sniff(iface=nic, filter=protocol, count=50, lfilter=lambda pkt: IP in pkt and (src_addr is None or pkt[IP].src == src_addr) and (dst_addr is None or pkt[IP].dst == dst_addr))
...
class MainFrame(tk.Frame):
    def __init__(self, master, packets):
        ...
        self.detail_frame = DetailFrame(self.canvas)
