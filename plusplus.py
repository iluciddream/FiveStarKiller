...
class MainFrame(tk.Frame):
    ...
    def show_next_data(self):
        if self.data_index < len(self.datas):
            ...
            self.after(5000, self.show_next_data)
...
class DataBar(tk.Frame):
    def __init__(self, master, data, detail_frame):
        ...
        self.text = tk.Text(self, height=1, wrap=tk.WORD, relief=tk.SOLID)
        self.text.insert(tk.END, data["name"])
        self.text.configure(state=tk.DISABLED)
        self.text.pack(side="left", fill=tk.BOTH, expand=True)
        self.text.bind("<Button-1>", self.show_detail)
    ...
    def show_next_data(self):
        if self.data_index < len(self.datas):
            data = self.datas[self.data_index]
            data_bar = DataBar(self.canvas, data, self.detail_frame)
            data_bar.update_idletasks()
            data_bar_height = data_bar.winfo_reqheight()
            self.canvas.create_window((0, len(self.data_bars) * data_bar_height), window=data_bar, anchor="nw", height=data_bar_height)
            self.data_bars.append(data_bar)
            self.data_index += 1
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))
            self.after(1000, self.show_next_data)
